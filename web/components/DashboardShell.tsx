...
export function DashboardShell({ companies }: Props) {
  ...
  return (
    <>
      {/* ── Stats grid (dark bg continues from header) ─────────── */}
      ...
      <div className="mx-auto max-w-7xl px-6 py-8">
        ...
        {/* Filter panel */}
        <div className="-mt-px rounded-b-2xl rounded-t-none border border-zinc-200 bg-white px-5 py-4 shadow-sm mb-6">
          <div className="flex items-center gap-3">
            <div className="relative flex-1">
              <svg viewBox="0 0 20 20" fill="currentColor" className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-zinc-400">
              ...
              <input
                ...
                className="w-full rounded-lg border border-zinc-200 bg-zinc-50 py-2 pl-9 pr-3 text-sm text-zinc-900 placeholder:text-zinc-400 focus:border-zinc-400 focus:bg-white focus:outline-none focus:ring-1 focus:ring-zinc-300 transition-colors"
              />
            </div>

            <label className="flex shrink-0 cursor-pointer items-center gap-2 rounded-lg border border-zinc-200 px-3 py-2 text-sm text-zinc-700 hover:bg-zinc-50 transition-colors select-none">
              <input
                type="checkbox"
                checked={vandyOnly}
                onChange={e => setVandyOnly(e.target.checked)}
                className="h-4 w-4 rounded border-zinc-300 accent-yellow-500"
              />
            ...
            {activeFilterCount > 0 && (
              <button
                onClick={clearAll}
                className="shrink-0 rounded-lg border border-zinc-200 px-3 py-2 text-xs text-zinc-500 hover:border-zinc-300 hover:text-zinc-700 transition-colors"
              >
        ...
        {filtered.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-zinc-300 bg-white p-16 text-center">
            ...
            <p className="font-medium text-zinc-700">No companies match these filters.</p>
            <p className="mt-1 text-sm text-zinc-500">Try widening your search or clearing a filter.</p>
          </div>
        ) : (
  ...
  return (
    <button
      ...
      className={`group w-full cursor-pointer px-6 py-5 text-left transition-colors duration-150 ${
        highlight
          ? active ? "bg-emerald-500/20" : "bg-emerald-500/10 hover:bg-emerald-500/20"
          : gold
    ...
    >
      ...
      >
        {label}
        {active && <span className="ml-1.5 text-zinc-500">·</span>}
        {active && <span className="ml-1 normal-case tracking-normal text-zinc-400">active</span>}
      </div>
    </button>
  );
}
