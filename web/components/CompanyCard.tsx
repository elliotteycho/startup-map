...
export function CompanyCard({ company: c }: { company: CompanyWithAlumni }) {
  ...
  return (
    <Link
      ...
      className={`group flex h-full flex-col rounded-xl border border-slate-700/40 border-l-[3px] ${accent} bg-slate-900/80 backdrop-blur-sm p-5 transition-all duration-200 hover:-translate-y-1 hover:border-slate-600/60 hover:bg-slate-900 hover:shadow-xl hover:shadow-violet-500/10`}
    >
      {/* Header row */}
      <div className="flex items-start gap-3">
        <CompanyAvatar name={c.name} website={c.website} />
        <div className="min-w-0 flex-1">
          <h3 className="truncate font-semibold text-slate-100 group-hover:text-white transition-colors">
            {c.name}
          </h3>
          {c.source_fund && (
      ...
      <div className="mt-auto pt-4 space-y-2.5">
        {c.location && (
          <p className="flex items-center gap-1.5 text-xs text-slate-500">
            <svg viewBox="0 0 20 20" fill="currentColor" className="h-3.5 w-3.5 shrink-0 text-slate-600">
